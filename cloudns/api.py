...
class api:
    ...
    def add_record(self, domain_name, record_type, host, record, ttl=3600,
        ...
        params = 'domain-name={}&record-type={}&host={}&record={}&ttl={}&status={}'.format(
            domain_name, record_type, host, record, ttl, status)
        logger.debug("Got request to create record - params: {}".format(params))
        if record_type == 'MX':
        ...
        if record_type == 'WR':
            ...
            params += '&frame={}&save-path={}&redirect-type={}'.format(
                frame, save_path, redirect_type)
    ...
    def copy_records(self, domain_name, from_domain, delete_current_records=0):
        params = 'domain-name={}&from-domain={}&delete_current_records={}'.format(
            domain_name, from_domain, delete_current_records)
    ...
    def add_mail_forward(self, domain_name, box, host, destination):
        params = 'domain-name={}&box={}&host={}&destination={}'.format(
            domain_name, box, host, destination)
    ...
    def modify_mail_forward(self, domain_name, box, host, destination, mail_forward_id):
        params = 'domain-name={}&box={}&host={}&destination={}&mail-forward-id={}'.format(
            domain_name, box, host, destination, mail_forward_id)
    ...
    def set_parked_settings(self, domain_name, template, title=None,
                            description=None, keywords=None, contact_form=0):
        args = locals()
        params = ''
        for k, v in args.items():
            if k == 'self':
